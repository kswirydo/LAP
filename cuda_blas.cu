...
void cuda_ichol(const int *ia, 
  ...
  if (sizeof(real_type) == 4) dataType = CUDA_R_32F;
  if (sizeof(real_type) == 8) dataType = CUDA_R_64F;
  cusparseCreateDnVec(&vecX, prec_data->n,  x, dataType);
  cusparseCreateDnVec(&vecY, prec_data->n,  y, dataType);
  cusparseCreateDnVec(&vecT, prec_data->n,  prec_data->aux_vec1, dataType);
